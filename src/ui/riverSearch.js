 ...
 *
 * `locateOnPick` — whether to read the mouth's coordinate out of the metadata store on a name pick.
 * The forecast tools want it, to make a found river savable; a named river's row already carries
 * everything the explorer's selection needs.
 * `requireLocationById` — whether an ID search must resolve to a coordinate. The charts need only
 * the riverIndex, but the explorer's upstream count comes out of that same read.
 */
