...
class Selection {
  ...
  addHighlightLayers() {
    ...
    line("flood-maps-unmappable", "#dc2626", 3, 0.95, null, FLOOD_MIN_MAP_ZOOM);
  ...
  select(riverIndex) {
    ...
    if (added && between.corridor.size > FLOOD_MAX_SEGMENTS) {
      this.clicked.delete(riverIndex);
      this.warn(`That reach would make ${between.corridor.size.toLocaleString()} reaches — the limit is ${FLOOD_MAX_SEGMENTS}. Selection unchanged.`);
      console.warn(`Rejected reach at index ${riverIndex}: corridor would span ${between.corridor.size} reaches (max ${FLOOD_MAX_SEGMENTS}).`);
      return;
    }
