/**
 * plots/yearlyVolumes.ts — annual discharge volume (Mm³) as a line, with a stepped 5-year
 * period-average line overlaid.
 */
import { Chart, axis, commonPlugins, chartCanvas, type ChartConfiguration } from './shared'
import type { RetroDerived } from './derive'

export function renderYearlyVolumes(host: HTMLElement, d: RetroDerived): Chart {
  const labels = d.yearlyVolumes.map((v) => v.year)
  const fiveYearFor = (year: number): number | null => {
